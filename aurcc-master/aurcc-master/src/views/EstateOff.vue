<template>
	<main class="flex-grow">
	  ...
	  <section class="mx-auto py-6 md:py-12 md:px-10 rounded-lg animate-popIn" 
		:style="{ backgroundImage: `url(${backgroundImageContent})`, backgroundSize: 'cover', backgroundPosition: 'center' }">
			...
			<div class="flex-shrink-0 w-full h-max md:w-64 bg-gradient-to-r from-[#21209c] to-blue-600 rounded-lg shadow-lg p-4 mb-4 md:mb-0 md:mr-4">
			  ...
			  <div class="space-y-2 font-serif">
				<button @click="currentSection = 'about'" 
				  :class="{'bg-yellow-400 text-[#23120b]': currentSection === 'about', 'bg-gray-100': currentSection !== 'about'}" 
				...
				<button @click="currentSection = 'services'" 
				  :class="{'bg-yellow-400 text-[#23120b]': currentSection === 'services', 'bg-gray-100': currentSection !== 'services'}" 
				  class="w-full py-2 px-4 rounded-md font-semibold hover:bg-yellow-400 hover:text-[#23120b]">
				  Services
				</button>
				<button @click="currentSection = 'members'" 
				  :class="{'bg-yellow-400 text-[#23120b]': currentSection === 'members', 'bg-gray-100': currentSection !== 'members'}" 
				  class="w-full py-2 px-4 rounded-md font-semibold hover:bg-yellow-400 hover:text-[#23120b]">
				  Members
				</button>
				<button @click="currentSection = 'contact'" 
				  :class="{'bg-yellow-400 text-[#23120b]': currentSection === 'contact', 'bg-gray-100': currentSection !== 'contact'}" 
				  class="w-full py-2 px-4 rounded-md font-semibold hover:bg-yellow-400 hover:text-[#23120b]">
				  Contact
				</button>
			  </div>
  ...
  import backgroundImageContent from '@/assets/aurcc_bg.webp'; // Add the correct path for the aurcc_bg image
