<template>
  <div class="flex-grow">
    ...
    <section class="relative w-full py-6 md:py-12 bg-cover bg-center animate-fadeInUp" :style="{ backgroundImage: `url(${aurcc_bg})` }">
      <div class="relative flex flex-col md:flex-row md:px-10 px-4">
        <!-- Vertical Tabs -->
        <div class="px-8 md:px-0">
          <div class="flex-shrink-0 w-full h-max md:w-64 bg-gradient-to-r from-[#21209c] to-blue-600 rounded-lg shadow-lg p-4 mb-4 md:mb-0 md:mr-4">
            ...
            <div class="space-y-2 font-serif">
              ...
              <button @click="currentSection = 'Recruitment Process'"
                      :class="tabClass('Recruitment Process')"
                      class="w-full py-2 px-4 rounded-md font-semibold hover:bg-yellow-400 hover:text-[#23120b]">
                Recruitment Process
              </button>
              <button @click="currentSection = 'CUIC'"
                      :class="tabClass('CUIC')"
                      class="w-full py-2 px-4 rounded-md font-semibold hover:bg-yellow-400 hover:text-[#23120b]">
                CUIC
              </button>
              <button @click="currentSection = 'Our Recruiters'"
                      :class="tabClass('Our Recruiters')"
                      class="w-full py-2 px-4 rounded-md font-semibold hover:bg-yellow-400 hover:text-[#23120b]">
                Our Recruiters
              </button>
              <button @click="currentSection = 'Placed Students'"
                      :class="tabClass('Placed Students')"
                      class="w-full py-2 px-4 rounded-md font-semibold hover:bg-yellow-400 hover:text-[#23120b]">
                Placed Students
              </button>
              <button @click="currentSection = 'Staff Members'"
                      :class="tabClass('Staff Members')"
                      class="w-full py-2 px-4 rounded-md font-semibold hover:bg-yellow-400 hover:text-[#23120b]">
                Staff Members
              </button>
            </div>
        ...
        <!-- Content Area -->
        <div class="w-full px-0 md:px-10 font-serif min-h-[400px] max-h-[600px] md:max-h-[800px] overflow-y-auto animate-fadeIn">
          <div v-if="currentSection === 'Description'" class="bg-white rounded-lg shadow-lg p-4 md:p-6 mb-16">
          ...
          <div v-else-if="currentSection === 'Recruitment Process'" class="bg-white rounded-lg shadow-lg p-4 md:p-6 mb-16">
          ...
          <div v-else-if="currentSection === 'CUIC'" class="bg-white rounded-lg shadow-lg p-4 md:p-6 mb-16">
            <h2 class="text-2xl md:text-3xl font-bold text-black mb-4">CUIC</h2>
            <p class="text-base md:text-lg mb-8 text-black">{{ data.cuic }}</p>
          </div>
          <div v-else-if="currentSection === 'Our Recruiters'" class="bg-white rounded-lg shadow-lg p-4 md:p-6 mb-16">
          ...
          <div v-else-if="currentSection === 'Placed Students'" class="bg-white rounded-lg shadow-lg p-4 md:p-6 mb-16">
          ...
          <div v-else-if="currentSection === 'Staff Members'" class="bg-white rounded-lg shadow-lg p-4 md:p-6 mb-16">
...
import aurcc_bg from '@/assets/aurcc_bg.webp';
...
export default {
  data() {
    return {
      ...
      recruitersPDF,
      placedStudentsImage,
      currentSection: 'Recruitment Process'
    };
  },
  methods: {
    tabClass(section) {
      return {
        'bg-yellow-400 text-[#23120b]': this.currentSection === section,
        'bg-gray-100 text-black': this.currentSection !== section
      };
    }
  }
};
